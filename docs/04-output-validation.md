...
The test suite covers all rejection paths:

| Scenario | Expected result |
|---|---|
| `WrongShapeModel` — returns one fewer prediction | 503, count mismatch |
| Model returns `None` for a prediction | 503, predictions[i] is None |
| Model returns `{"score": float("nan")}` | 503, non-finite value at predictions[0].score |
| Model returns `{"score": float("inf")}` | 503, non-finite value at predictions[0].score |
| Nested bad value `{"details": {"score": float("nan")}}` | 503, non-finite value at predictions[0].details.score |

## No Regressions

The full test suite (62 tests) passes. The `_check_finite` traversal is O(predictions × fields) — negligible at typical batch sizes.
