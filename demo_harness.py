...
def main() -> None:
    ...
    print(f"wall clock total : {wall * 1000:>7.1f} ms")
    ...
    # Parallel: wall ≈ baseline (1x). Serial: wall ≈ n * baseline.
    # Threshold of 3x gives clear headroom for scheduling noise.
    if ratio > 3:
        print(f"\nVERDICT: looks SERIALISED (wall = {ratio:.1f}x single-call baseline)")
    else:
        print(f"\nVERDICT: looks PARALLEL (wall = {ratio:.1f}x single-call baseline)")
